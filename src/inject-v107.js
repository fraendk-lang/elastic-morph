...
function handoffExportToUniversePostReady(blob) {
  if (!blob || !(blob instanceof Blob)) return false;
  const file = new File([blob], "elastic-morph-export.mp4", { type: blob.type || "video/mp4" });
  try {
    window.parent.postMessage({ type: "elastic-universe:morph-export", file }, location.origin);
    return true;
  } catch (e) {
    console.warn("Morph → Post-Ready handoff failed:", e);
    return false;
  }
}

function openPostReadyAfterExport() {
  const blob = window.__lastExportBlob;
  if (isUniverseEmbed() && blob && handoffExportToUniversePostReady(blob)) {
    closeExportSuccess();
    showAppToast("Weiter zu Post-Ready — Export wird übernommen…", 4500);
    return;
  }
